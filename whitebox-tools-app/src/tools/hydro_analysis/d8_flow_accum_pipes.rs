...
/// optional `--esri_pntr` flag. Pipes (`--pipes`) can also be provided in order to catch accumulation at a given point
/// and transport it directly at another. This allow to take into account underground canalisation without affecting
/// DEM values.
///
...
pub struct D8FlowAccumulationPipes {
    name: String,
    description: String,
    toolbox: String,
...
impl D8FlowAccumulationPipes {
    pub fn new() -> D8FlowAccumulationPipes {
        // public constructor
        let name = "D8FlowAccumulationPipes".to_string();
        ...
        let usage = format!(">>.*{0} -r={1} -v --wd=\"*path*to*data*\" --input=DEM.tif -o=output.tif --pipes='pipes.shp' --out_type='cells'
>>.*{0} -r={1} -v --wd=\"*path*to*data*\" --input=DEM.tif -o=output.tif --out_type='specific catchment area' --log --clip", short_exe, name).replace("*", &sep);

        D8FlowAccumulationPipes {
            name: name,
...
impl WhiteboxTool for D8FlowAccumulationPipes {
    ...
    fn run<'a>(
        ...
        while !set_idx_pipes.is_empty() {
            println!("Pipes remaining: {}", set_idx_pipes.len());
