...
fn get_downslope_index<'a>(dem: &'a Raster, max_drop:f64, max_dist:f64) -> Array2D<f32> {
    let rows = dem.configs.rows as isize;
    let columns = dem.configs.columns as isize;
    let nodata = dem.configs.nodata;
    ...
    let fdir_noflow = -1_i8;
    let mut d8: Array2D<i8> = Array2D::new(rows, columns, fdir_noflow, fdir_noflow).unwrap();
    ...
    for tid in 0..num_procs {
        ...
        thread::spawn(move || {
            ...
            for row in (0..rows).filter(|r| r % num_procs == tid) {
                let mut data = vec![fdir_noflow; columns as usize];
                for col in 0..columns {
                    z = dem[(row, col)];
                    if z != nodata {
                        let mut dir = 0;
